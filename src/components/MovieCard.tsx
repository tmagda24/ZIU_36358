...
export function MovieCard({ movie, onClick }: Props) {
  ...
  const handleToggle = useCallback(async (e: React.MouseEvent) => {
    ...
    try {
      ...
      // ZADANIE A — zdarzenie niestandardowe #1: kliknięcie CTA (konwersja).
      // Dodanie filmu do ulubionych to główna akcja konwersji aplikacji.
      // RODO (minimalizacja): wysyłamy tylko etykietę akcji i miejsce kliknięcia.
      // NIE wysyłamy tytułu ani id filmu (nie są potrzebne do oceny UX kart).
      trackEvent('CTA Click', {
        action: displayedFav ? 'remove_favorite' : 'add_favorite',
        location: 'movie_card',
      });
